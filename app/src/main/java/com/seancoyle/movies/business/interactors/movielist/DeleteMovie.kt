...
class DeleteMovie<ViewState>(
    ...
    fun execute(
        movies: MoviesDomainEntity,
        stateEvent: StateEvent
    ): Flow<DataState<ViewState>?> = flow {

        val cacheResult = safeCacheCall(IO) {
            cacheDataSource.deleteById(movies.id)
        }
    ...
    companion object {
        val DELETE_MOVIE_SUCCESS = "Successfully deleted movie."
        val DELETE_MOVIE_FAILED = "Failed to delete movie."
    }
}
