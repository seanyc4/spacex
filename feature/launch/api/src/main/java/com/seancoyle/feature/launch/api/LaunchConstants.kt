package com.seancoyle.feature.launch.api

object LaunchConstants {
    const val LAUNCH_OPTIONS_ROCKET = "rocket"
    const val LAUNCH_OPTIONS_SORT = "desc"
    const val DEFAULT_LAUNCH_THUMBNAIL = "https://thespacedevs-dev.nyc3.digitaloceanspaces.com/media/images/255bauto255d__image_thumbnail_20240305193746.jpeg"
    const val DEFAULT_LAUNCH_IMAGE = "https://thespacedevs-dev.nyc3.digitaloceanspaces.com/media/images/gslv2520mk2520iii_image_20190604000938.jpg"
    const val LAUNCH_EXCEPTION = -1
    const val PAGINATION_LIMIT = 20
    const val INITIAL_LOAD_SIZE = 20
    const val PREFETCH_DISTANCE = 10
    const val LAUNCH_API_PAGE = "launch_current_page"
    const val LAUNCH_CACHE_END_REACHED = "launch_cache_end_reached"
}
