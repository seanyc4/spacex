...
class FakeGetLaunchesApiAndCacheUseCase : GetLaunchesApiAndCacheUseCase {
    ...
    val isSuccess = true
    override fun invoke(currentPage: Int): Flow<LaunchResult<List<LaunchTypes.Launch>, DataError>> = flow {
        emit(getFakeLaunchesFromNetwork())
    }

    private fun getFakeLaunchesFromNetwork(): LaunchResult<List<LaunchTypes.Launch>, DataError> {
        // Simulating network data fetching behavior
        return if (shouldSimulateSuccess(isSuccess)) {
            // Simulate a successful data fetch
            LaunchResult.Success(emptyList())
        } else {
