...
class InsertLaunchesToCacheUseCaseImplTest {
    ...
    fun setup() {
        ...
        underTest = InsertLaunchesToCacheUseCaseImpl(cacheDataSource)
    }
    ...
    fun `invoke should return success when data source succeeds`() = runTest {
        ...
        coEvery { cacheDataSource.insertList(launchesModel) } returns insertResult
    ...
    fun `invoke should return error when data source fails`() = runTest {
        ...
        coEvery { cacheDataSource.insertList(launchesModel) } returns errorResult
